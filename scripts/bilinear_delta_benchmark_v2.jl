...
balance constraints. Compares five bilinear approximation methods from
InfrastructureOptimizationModels:

  1. Solver-native SOS2 (Bin2)
  2. Manual SOS2 (Bin2, binary-variable adjacency)
  3. Sawtooth MIP (Bin2)
  4. HybS (Hybrid Separable, Bin2 lower + Bin3 upper)
  5. D-NMDT (Doubly Discretized Normalized Multiparametric Disaggregation)

Usage:
    julia --project=test scripts/bilinear_delta_benchmark_v2.jl [N] [K] [seed]
...
# ─── Expression type for retrieving bilinear results ─────────────────────────

"""Map method symbol to the expression type used to store results."""
function _get_expression_type(method::Symbol)
    if method in (:sos2, :manual_sos2, :sawtooth)
...
- `method::Symbol`: `:sos2`, `:manual_sos2`, `:sawtooth`, `:hybs`, or `:dnmdt`
- `refinement::Int`: number of PWL segments (SOS2) or depth (sawtooth/hybs/dnmdt)
"""
function build_mip_model(net::NetworkData, method::Symbol, refinement::Int)
    ...
    elseif method === :dnmdt
        ...
        error("Unknown method: $method. Use :sos2, :manual_sos2, :sawtooth, :hybs, or :dnmdt.")
    end
    ...
    # Retrieve expression containers from the optimization container
    expr_type = _get_expression_type(method)
    z_gen = IOM.get_expression(container, expr_type, NetworkNode, "gen")
    z_dem = IOM.get_expression(container, expr_type, NetworkNode, "dem")
...
"""
    run_benchmark(; N, K, seed, segment_counts, depths)

Run the full benchmark comparing all five bilinear approximation methods.
...
- `depths::Vector{Int} = [2, 4, 8]`: depths for sawtooth, HybS, and D-NMDT methods
"""
function run_benchmark(;
    ...
    println("  Refinement = num_segments for SOS2 methods, depth for Sawtooth/HybS/D-NMDT")
    println("=" ^ 100)
    @printf("%-14s %5s %7s %8s %7s %12s %9s %10s %10s %10s\n",
        "Method", "Ref", "Vars", "Constrs", "Bins", "Objective",
        "Gap(%)", "Max Resid", "Build(s)", "Solve(s)")
    println("-" ^ 100)
        ...
        for ref in refs
            ...
            if status in (JuMP.OPTIMAL, JuMP.TIME_LIMIT) &&
               JuMP.has_values(result.jump_model)
                ...
                @printf("%-14s %5d %7d %8d %7d %12.6f %s %10.2e %10.4f %10.4f\n",
                    label, ref,
                    sz.variables, sz.constraints, sz.binaries,
                    obj, gap_str, maximum(resids), build_t, solve_t)
            else
                @printf("%-14s %5d %7d %8d %7d %12s %9s %10s %10.4f %10.4f\n",
                    label, ref,
                    sz.variables, sz.constraints, sz.binaries,
